...
- **대표/추가 이미지는 로컬 파일명(basename)을 그대로 셀에 쓴다.** WING '이미지(파일) 업로드' 창에서 업로드한
  원본 파일명이 그대로 표시/복사되는 것을 실제 화면으로 확인함 (로컬 파일명 = WING 파일명). 혹시 나중에
  카테고리별로 다르게 동작하는 게 확인되면 `excel_writer.py`의 이 부분만 바꾸면 된다.
