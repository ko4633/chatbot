...
- **대표/추가 이미지 컬럼은 일부러 비워둔다.** WING에 이미지를 업로드하면 로컬 파일명과 다른 파일명/경로가
  생성되기 때문에, 로컬 파일명을 셀에 써넣으면 오히려 틀린 값이 들어간다. 그래서 이미지는 사용자가
  WING에 먼저 업로드하고, 거기서 나온 파일명/URL을 최종 엑셀에 직접 붙여넣는 방식으로 확정했다
  (`excel_writer.py`는 이 컬럼을 건드리지 않는다).
