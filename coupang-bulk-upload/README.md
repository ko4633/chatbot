...
- **라벨(소재/제조국) 사진은 고정 인덱스(`_8`,`_9`)가 아니라 "그 상품 사진 중 항상 마지막 2장"이다.**
  실사용 확인: 어떤 상품은 9장(라벨=`_8`,`_9`), 어떤 상품은 7장(라벨=`_6`,`_7`)이었음. `image_grouper.py`는
  각 품번의 사진 개수를 보고 마지막 2장을 라벨로, 그 앞은 전부 추가이미지로 분류한다. 사진이 3장 미만이면
  라벨을 구분할 수 없다는 WARNING만 남기고 자동판별을 포기한다.
